...
(function() {
    ...
    // Your code here...
    // https://vshare.io/404?url=https://s902.vshare.io:440/s,128-1000-22-1-2191707-bkxdtksrwj/186745/351673/185658/ff-8840c3e48fc8f80f67eeacc4b3fc3cdbb94c86b4,5c584cc5,2f23e49_480.mp4
    window.stop();
    // ---------------------------------------------------
    ...
    addGlobalStyle(
        prettyPrint('body {position: absolute; width: 100%; height: 100%; overflow: hidden; padding: 0px; margin: 0px; top: 0px; left: 0px; background: black;}') +
        '\n' + prettyPrint('video {position: absolute; width: 100%; height: 100%; max-height: 100%; max-width: 100%; background: black; padding: 0px; margin: 0px; top: 0px; left: 0px; background: black;}'),
        'pageStyle'
    )
    // ---------------------------------------------------
...
function initPlayer() {
	...
	var paramStart = 'player.html#'; // '://vshare.io/v/404/';
	...
	if (url && typeof url !== 'undefined') {
		url = url.replace(/[?#&]\bREFINE_VIDEO\b/, '');
	}

	function listParams(obj) {
		for(var i in obj){
			console.log(i + "=" + obj[i]);
		}
	}
	...
	if (url) {
		...
		if (video) {
			var href = location.href.split(paramStart)[1];
			var params = getParamsFromURL(href); // getParamsFromURL(location.search)
			// console.log('params: ', params);
			listParams(params);
			if (params.autoplay && (params.autoplay == 'true' || params.match('autoplay=true'))) video.setAttribute('autoplay', '');
			...
			useLocalVolumeCookie("body > video", "video"); // useGMVolumeCookie("body > video", "video");
		}
	}
};
initPlayer();
