...
(function() {
    ...
    // --------------------------------------------------------------------------------
    var G_embedCodeVideo, G_sampleURL, G_videoWidth, G_videoHeight, G_videoQuality, G_forceLoad; function addEmbedCodeVideo(embedCodeFrame, onClickFunc, onLoadFunc, onErrorFunc, forceLoad = false) {
    ...
    var G_noPlayerExtension = false, G_standartReCastFunc = function() {
        var contentURL = G_contentURL ? G_contentURL : G_funcResult.src;
    ...
    var G_noQualitySample = false, G_qualitySampleSource = null, G_queryURL = null, G_standartAddEmbedCodeFunc = function() {
        ...
        if (G_qualitySampleSource) {
            ...
            G_qualitySampleSource.addEventListener('playing', function(e) {
                ...
                //
                G_embedCodeText = updateEmbedCodeText(G_embedCodeTextArea, 1, G_delimiter);
                G_embedCodeTextAreaColor = valToColor((G_videoWidth * G_videoHeight) / (1920 * 1080) * 100, 1, 1.0, 0, 100, 1);
            ...
            if (G_sampleURL && !G_noQualitySample) {
                G_embedCodeVideo = addEmbedCodeVideo(G_embedCodeFrame, G_funcToRun, function onLoadFunc() {
                    ...
                    G_embedCodeTextArea.style.color = G_embedCodeTextAreaColor;
                    log(G_debugMode, G_embedCodeTextAreaColor);
                    // alert(G_videoWidth+'x'+G_videoHeight+' : '+((G_videoWidth * G_videoHeight) / (1920 * 1080) * 100));
    ...
    // ================================================================================
        ...
        G_pageURL.matchLink('https?://www.pornhub.com/*')
    ) {
        // --------------------------------------------------------------------------------
        var actualSource = () => {
            var contentURL;
        ...
        // --------------------------------------------------------------------------------
            ...
            G_pageURL.matchLink('https?://www.pornhub.com/view_video.php[?]viewkey=*') // https://www.pornhub.com/view_video.php?viewkey=ph5852ef85649df
        ) {
            G_funcToRun = function () {
                ...
                G_standartAddEmbedCodeFunc();
                setTimeout(function() {
                    var eventCatcher = document.querySelector('div.mhp1138_eventCatcher'), media = document.querySelector('video');
                    if (eventCatcher && media) mediaMouseControls(eventCatcher, media, 1);
                }, 1000);
            };
            document.addEventListener("DOMContentLoaded", function(event) {
        ...
        if (
            ...
            document.addEventListener("DOMContentLoaded", function(event) {
                for (var button of document.querySelectorAll('.controls > .r > a[href^="#server"]')) {
    ...
    // ================================================================================
        ...
        if (
            ...
            G_funcToRun = function () {
                var imagesArray = [], thumbsArray = [];
                for (var link of document.querySelectorAll('div.gallery div.thumbnails a[href]')) {
                    var imageURL = link.href, img = link.querySelector('img');
                    if (img) {
                        var thumbURL = img.src;
                        thumbsArray.push(thumbURL);
